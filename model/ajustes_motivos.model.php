...
class AjustesMotivosModel {
    ...
    public function getCountAjustesMotivos(){
        try {
            $sql = "SELECT count(*) FROM ajustes_motivos;";
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM ajustes_motivos WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteAjustesMotivo($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from ajustes_motivos where codigo = ? and codigo not in (select cod_ajuste_motivo from ajustes)');
    ...
    public function addAjustesMotivo($descripcion, $impacto){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('INSERT INTO ajustes_motivos (descripcion, impacto, usuario_m, fecha_m) VALUES (?,?,?,?);');
            $stmt->bindValue(1, $descripcion, PDO::PARAM_STR);
            $stmt->bindValue(2, $impacto, PDO::PARAM_INT);
    ...
    public function updateAjustesMotivo($codigo, $descripcion, $impacto){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE ajustes_motivos set '
                                            . 'descripcion = ? , '
                                            . 'impacto = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
    ...
    public function getAjustesMotivo($codigo){
        try {
            $sql = "SELECT * FROM ajustes_motivos WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
