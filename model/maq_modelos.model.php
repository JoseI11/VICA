...
class MaqModelosModel {
    ...
    public function getCountMaqModelos(){
        try {
            $sql = "SELECT count(*) FROM maquinas_modelos;";
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM maquinas_modelos WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteMaqModelo($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from maquinas_modelos where codigo = ? and codigo not in (select cod_maquina_modelo from maquinas)');
    ...
    public function addMaqModelo($descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('INSERT INTO maquinas_modelos (descripcion, usuario_m, fecha_m) VALUES (?,?,?);');
    ...
    public function updateMaqModelo($codigo, $descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE maquinas_modelos set '
                                            . 'descripcion = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
                                            . ' where codigo = ?');            
    ...
    public function getMaqModelo($codigo){
        try {
            $sql = "SELECT * FROM maquinas_modelos WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
