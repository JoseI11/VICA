...
class FormasModel {
    ...
    public function getCountFormas(){
        try {
            $sql = "SELECT count(*) FROM forma_pagos;";
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM forma_pagos WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteForma($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from forma_pagos where codigo = ? 
                and codigo not in (select cod_forma_pago from orden_ventas_pagos) 
            ');
    ...
    public function addForma($descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('INSERT INTO forma_pagos (descripcion, usuario_m, fecha_m) VALUES (?,?,?);');
    ...
    public function updateForma($codigo, $descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE forma_pagos set '
                                            . 'descripcion = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
                                            . ' where codigo = ?');            
    ...
    public function getForma($codigo){
        try {
            $sql = "SELECT * FROM forma_pagos WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
