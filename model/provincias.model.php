...
class ProvinciasModel {
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM provincias WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteProvincia($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from provincias where codigo = ? 
                and codigo not in (select cod_provincia from clientes) and codigo not in (select cod_provincia from proveedores)
                and codigo not in (select cod_provincia from transportes) and codigo not in (select cod_provincia from vendedores)
            ');
    ...
    public function addProvincia($descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('INSERT INTO provincias (descripcion, usuario_m, fecha_m) VALUES (?,?,?);');
    ...
    public function updateProvincia($codigo, $descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE provincias set '
                                            . 'descripcion = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
                                            . ' where codigo = ?');            
