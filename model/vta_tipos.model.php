...
class VtaTiposModel {
    ...
    public function getCountVtaTipos(){
        try {
            $sql = "SELECT count(*) FROM orden_ventas_tipos;";
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM orden_ventas_tipos WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteVtaTipo($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from orden_ventas_tipos where codigo = ? and codigo not in (select cod_orden_venta_tipo from orden_ventas)');
    ...
    public function addVtaTipo($descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('INSERT INTO orden_ventas_tipos (descripcion, usuario_m, fecha_m) VALUES (?,?,?);');
    ...
    public function updateVtaTipo($codigo, $descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE orden_ventas_tipos set '
                                            . 'descripcion = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
                                            . ' where codigo = ?');            
    ...
    public function getVtaTipo($codigo){
        try {
            $sql = "SELECT * FROM orden_ventas_tipos WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
