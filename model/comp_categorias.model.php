...
class CompCategoriasModel {
    ...
    public function getCountCompCategorias(){
        try {
            $sql = "SELECT count(*) FROM componentes_categorias;";
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM componentes_categorias WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteCompCategoria($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from componentes_categorias where codigo = ? and codigo not in (select cod_componente_categoria from componentes)');
    ...
    public function addCompCategoria($descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('INSERT INTO componentes_categorias (descripcion, usuario_m, fecha_m) VALUES (?,?,?);');
    ...
    public function updateCompCategoria($codigo, $descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE componentes_categorias set '
                                            . 'descripcion = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
                                            . ' where codigo = ?');            
    ...
    public function getCompCategoria($codigo){
        try {
            $sql = "SELECT * FROM componentes_categorias WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
