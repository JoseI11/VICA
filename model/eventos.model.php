...
class EventosModel {
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM eventos WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteEvento($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from eventos where codigo = ? 
                and codigo not in (select cod_evento from orden_trabajo_eventos) 
            ');
    ...
    public function addEvento($descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('INSERT INTO eventos (descripcion, usuario_m, fecha_m) VALUES (?,?,?);');
    ...
    public function updateEvento($codigo, $descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE eventos set '
                                            . 'descripcion = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
                                            . ' where codigo = ?');            
