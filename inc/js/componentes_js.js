...
$("#guardarDatosComponente").submit(function (event) {
    if (!requestSent) {
        requestSent = true;
        var parametros = {
            funcion: "addComponente",
            descripcion: $("#descripcionAdd").val(),
            insumo: $("#insumoAdd").val(),
            usado: $("#usadoAdd").val(),
            codigo_mp: $("#CodigoAdd").val(),
            unidad: $("#unidadAdd").val(),
            dimension: $('#DimensionAdd').val(),
            categoria: $('#CategoriaAdd').val(),
            costo: $('#CostoAdd').val(),
            precio: $('#PrecioAdd').val(),
            stock_min: $('#StockMinAdd').val(),
            stock_max: $('#StockMaxAdd').val(),
            iva: $('#IvaAdd').val(),
            espesor: $("#EspesorAdd").val(),
            largo: $("#LargoAdd").val(),
            largo_total: $("#LargoTotalAdd").val(),
            peso: $("#PesoAdd").val(),
            peso_total: $("#PesoTotalAdd").val()
        }
        $.ajax({
            type: "POST",
            url: 'controller/componentes.controller.php',
